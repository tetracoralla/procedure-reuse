#!/usr/bin/env node
/**
 * Thin MCP stdio carrier around preflight.mjs.
 *
 * Tool asset_delivery_preflight calls runPreflight. Observation remains
 * org.openadam.file.inspect@0.1.0 (File Vitals). This carrier does not
 * implement raster.verify and does not contain a scaffold stub.
 */
...
export const TOOL_NAME = "asset_delivery_preflight";
export const SERVER_NAME = "asset-delivery-preflight";
...
export const INPUT_SCHEMA = {
  ...
  properties: {
    root: {
      ...
      minLength: 1,
      maxLength: 4096,
      description: "Delivery directory relative to the workspace grant. Slot paths are relative to this directory. File Vitals inspect uses this resolved directory as its grant.",
    },
    specPath: {
      type: "string",
      minLength: 1,
    ...
    spec: {
      ...
      additionalProperties: false,
      required: ["slots"],
      description: "Inline slot spec. Provide exactly one of spec or specPath.",
      properties: {
        id: { type: "string" },
        version: { type: "string" },
        title: { type: "string" },
        description: { type: "string" },
        alphaPolicy: { type: "object" },
...
export const TOOL_DEFINITION = {
  name: TOOL_NAME,
  title: "Asset delivery preflight",
  description: "Read-only suite preflight for a slot spec and delivery directory. Observes present files with org.openadam.file.inspect@0.1.0 inspect (File Vitals) and compares name, format, width, height, alpha, missing, and extra in ordinary code. Does not implement raster.verify.",
...
export function initializeResult(params) {
  ...
  return {
    ...
    instructions: "Call asset_delivery_preflight with a workspace grant, a relative delivery root, and exactly one of spec or specPath. Observation is file.inspect; comparison is combinator code, not raster.verify.",
  };
}
