...
test("MCP catalog advertises a closed read-only preflight tool", async () => {
...
whenAdapter("good delivery through MCP calls runPreflight and passes", async () => {
  await withGrant(async () => {
    ...
    assert.equal(result.structuredContent.observer.operation, "inspect");
    assert.match(result.structuredContent.observer.note, /not raster.verify/i);
  });
});
...
whenAdapter("stdio initialize then tools/list then a real preflight call", async () => {
  ...
  assert.equal(stderr, "");
  assert.equal(code, 0);
  assert.equal(messages[0].result.serverInfo.name, "asset-delivery-preflight");
